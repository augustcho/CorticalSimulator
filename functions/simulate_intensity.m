function results = simulate_intensity(config)
%SIMULATE_INTENSITY Run cortical simulation varying stimulus intensity
...
for inten_iter = 1:length(config.stimulus_intensities)
    ...
    for tr = 2:config.end_sec/2-1
        ...
        data_3D(tp:tp+config.stim_duration-1,:,tr) = ...
            data_3D(tp:tp+config.stim_duration-1,:,tr) + stimulus_intensity;
