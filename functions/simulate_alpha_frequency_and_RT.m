function results = simulate_alpha_frequency_and_RT(config)
%SIMULATE_ALPHA_FREQUENCY Run alpha-frequency simulation.

srate = config.parameters.srate;
times = linspace(0, config.end_sec, srate*config.end_sec);
n_tp = length(times);

m_rts = nan(length(config.osc_hz_list), config.n_reps);
s_rts = nan(length(config.osc_hz_list), config.n_reps);
response_rate = nan(length(config.osc_hz_list), config.n_reps);

for re_iter = 1:config.n_reps
    fprintf('iter = %d\n', re_iter);

    for hz_iter = 1:length(config.osc_hz_list)
        osc_hz = config.osc_hz_list(hz_iter);
        ...
        for tr = 2:config.end_sec/2-1
            ...
            data_3D_real(tp:tp+config.stim_duration,:,tr) = ...
        ...
        for iter = 1:13
            CTX{iter} = cortex();
            rand_hz = osc_hz - 0.5 + rand(1); % small jitter
        ...
        CTX{1}.insertAfter(CTX{2},dly);
        CTX{2}.insertAfter(CTX{3},dly);
        CTX{3}.insertAfter(CTX{7},10);

        CTX{4}.insertAfter(CTX{5},dly);
        CTX{5}.insertAfter(CTX{6},dly);
        CTX{6}.insertAfter(CTX{7},10);

        CTX{7}.insertAfter(CTX{8},dly);
        CTX{8}.insertAfter(CTX{9},dly);
        CTX{9}.insertAfter(CTX{10},10);

        CTX{10}.insertAfter(CTX{11},dly);
        CTX{11}.insertAfter(CTX{12},dly);
        ...
        true_rt = rt(rt > 0);
        if ~isempty(true_rt)
            m_rts(hz_iter,re_iter) = mean(true_rt);
            s_rts(hz_iter,re_iter) = std(true_rt)/sqrt(numel(true_rt));
            response_rate(hz_iter,re_iter) = numel(true_rt)/n_trials*100;
        end
    end
end

% ---- Save results ----
results.osc_hz_list   = config.osc_hz_list;
results.m_rts         = m_rts;
results.s_rts         = s_rts;
